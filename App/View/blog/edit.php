...
ob_start();
?>
<form action="index.php?action=post-confirm_edit-<?= $post_data['post']->getId(); ?>" method="post">
    <div class="container">
        <p class="post_line_header mt-4 mb-4">
            <input class="post_input" type="text" id="post_input_title" name="title_input" value="<?= $post_data['post']->getTitle(); ?>" required="required">
        </p>
    </div>
    <div class="container">
        ...
        <p class="post_line_header mt-4">
            <input class="post_input" type="text" id="post_input_header" name="header_input" value="<?= $post_data['post']->getHeader(); ?>" required="required">
        </p>
        <p class="post_line_content mt-4">
            <textarea class="post_input" name="content_input"
                      rows="20" required="required"><?= $post_data['post']->getContent(); ?></textarea>
        </p>
    ...
    <h3 class="mt-3 mb-3">&#10098;Comments&#10099;</h3>
        <?php foreach ($post_data['comments'] as $comment) { ?>
            <p class="comments_line">
                <a href="index.php?action=comment-<?= ($comment->getVerified() === 1?'invalidate':'validate') . '-' . $comment->getId(); ?>" class="post_link_a post_link_edit">
                    <?= ($comment->getVerified() === 1?'&#9745;':'&#9744;'); ?>
                </a>
                <a href="#" onclick="alertMsg(<?= $comment->getId(); ?>, 'comment')" class="post_link_a post_link_delete" style="margin-right: 0.5em;">&#10007;</a>
                <span class="comment_infos"><?= '[' . $comment->getCreatedDate() . ']' . htmlspecialchars($comment->getAuthor()->getUsername()); ?>:</span>
