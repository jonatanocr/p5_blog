...
ob_start();
?>
<h1 class="mt-4 mb-4"><?= $post_data['post']->getTitle(); ?></h1>
<div class="row">
    <p class="post_line_header"><?= '[' . $post_data['post']->getUpdatedDate() . '] ' . $post_data['post_author']->getUsername(); ?></p>
    <p class="post_line_header"><?= $post_data['post']->getReadingTime() . 'min · ' . $post_data['post']->getHeader(); ?></p>
...
<div class="row comments_div">
    ...
    <?php foreach ($post_data['comments'] as $comment) {
        if ($comment->getVerified() === 1) { ?>
            ...
            <?= '[' . $comment->getCreatedDate() . '] ' . htmlspecialchars($comment->getAuthor()->getUsername()) . ':</span> ' . htmlspecialchars($comment->getContent()); ?>
            </p>
        <?php }
    } ?>

    <form action="index.php?action=comment-add_comment-<?= $post_data['post']->getId(); ?>" method="post">
        <div class="container">
